/**
 * Message-send socket layer (wraps makeNewsletterSocket).
 * Faithful port of upstream Socket/messages-send.js: relayMessage builds the exact stanza WhatsApp
...
import { BIZ_BOT_SUPPORT_PAYLOAD, DEFAULT_CACHE_TTLS, WA_DEFAULT_EPHEMERAL } from '../defaults.js'
import { AssociationType } from '../constants.js'
import {
	bindWaitForEvent,
	delay,
...
import { assertMeId, buildMergedTcTokenIndexWrite, isTcTokenExpired, resolveIssuanceJid, resolveTcTokenJid, shouldSendNewTcToken, storeTcTokensFromIqResult } from '../utils/auth-state-core.js'
import {
	areJidsSameUser,
	getBinaryNodeChild,
    ...
    const fullSock = {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                // otherwise resend stale content after this succeeds (see messageRetryManager
                // in messages-recv.js). Only invalidate once the edit/revoke actually went out.
                if (messageRetryManager && (isDeleteMsg || isEditMsg)) {
                    const targetKey = isDeleteMsg ? content.delete : content.edit;
                    if (targetKey?.id) {
                        messageRetryManager.invalidateMessage(targetKey.remoteJid || jid, targetKey.id);
                    }
                }
                if (config.emitOwnEvents) {
                    process.nextTick(async () => {
...
export { AssociationType, BIZ_BOT_SUPPORT_PAYLOAD, isJidBot, isJidMetaAI, PSA_WID, shouldSendNewTcToken, resolveIssuanceJid, resolveTcTokenJid, isTcTokenExpired, buildMergedTcTokenIndexWrite, assertMeId, delay, isHostedLidUser, isHostedPnUser, isLidUser, isPnUser, generateParticipantHashV2, hasValidAlbumMedia, encodeNewsletterMessage, DEF_MEDIA_HOST, makeMutex }
