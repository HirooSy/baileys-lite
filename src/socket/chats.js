...
import { makeMutex } from '../foundation/concurrency.js'
import { DEFAULT_CACHE_TTLS, HISTORY_SYNC_PAUSED_TIMEOUT_MS, PROCESSABLE_HISTORY_TYPES } from '../defaults.js'
import { ALL_WA_PATCH_NAMES, SyncState } from '../constants.js'
import {
	...
	newLTHashState,
	processSyncAction
} from '../utils/chat-sync.js'
import { isAppStateSyncIrrecoverable, isMissingKeyError, MAX_SYNC_ATTEMPTS } from '../utils/chat-sync.js'
import { generateProfilePicture } from '../utils/media.js'
import processMessage, { getHistoryMsg } from '../utils/message-processing.js'
import { buildTcTokenFromJid } from '../utils/auth-state-core.js'
import {
	getBinaryNodeChild,
...
import { makeSocket } from './socket-core.js'
...
export const makeChatsSocket = config => {
	...
	const placeholderResendCache = config.placeholderResendCache || new Cache({ ttl: DEFAULT_CACHE_TTLS.MSG_RETRY * 1000 })
	...
	const getBotListV2 = async () => {
		...
		for (const section of getBinaryNodeChildren(botNode, 'section')) {
			if (section.attrs.type === 'all') {
				for (const bot of getBinaryNodeChildren(section, 'bot')) botList.push({ jid: bot.attrs.jid, personaId: bot.attrs['persona_id'] })
			}
		}
		return botList
	}
	...
	const updateProfilePicture = async (jid, content, dimensions) => {
		...
		targetJid = jidNormalizedUser(jid) !== jidNormalizedUser(authState.creds.me.id) ? jidNormalizedUser(jid) : undefined
			...
			while (collectionsToHandle.size) {
				...
				for (const key in decoded) {
					...
					try {
						...
						if (hasMorePatches) logger.info(`${name} has more patches...`)
						else collectionsToHandle.delete(name)
					} catch (error) {
	...
	const profilePictureUrl = async (jid, type = 'image', timeoutMs = 5000, shouldIncludeTcToken = true) => {
		...
		const result = await query(
			{ tag: 'iq', attrs: { target: jid, to: S_WHATSAPP_NET, type: 'get', xmlns: 'w:profile:picture' }, content: buildProfilePictureQueryContent(type, tcTokenContent) },
			timeoutMs
		)
	...
	const createCallLink = async (type, event, timeoutMs) => {
		const result = await query(
			{ tag: 'call', attrs: { id: generateMessageTag(), to: '@call' }, content: [{ tag: 'link_create', attrs: { media: type }, content: event ? [{ tag: 'event', attrs: { start_time: String(event.startTime) } }] : undefined }] },
			timeoutMs
		)
					...
					content: [
						{
							tag: 'sync',
							attrs: {},
							content: [{ tag: 'collection', attrs: { name, version: (state.version - 1).toString(), return_snapshot: 'false' }, content: [{ tag: 'patch', attrs: {}, content: proto.SyncdPatch.encode(patch).finish() }] }]
						}
					]
				}
				await query(node)
	...
	const fetchProps = async () => {
		const resultNode = await query({
			tag: 'iq',
			attrs: { to: S_WHATSAPP_NET, xmlns: 'abt', type: 'get' },
			content: [{ tag: 'props', attrs: { protocol: '1', ...(authState?.creds?.lastPropHash ? { hash: authState.creds.lastPropHash } : {}) } }]
		})
	...
	const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
		...
		await Promise.all([
			(async () => {
				if (shouldProcessHistoryMsg) await doAppStateSync()
			})(),
			processMessage(msg, {
				signalRepository,
