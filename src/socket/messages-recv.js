...
import { ReachoutTimelockEnforcementType, WAMessageStatus, WAMessageStubType } from '../constants.js'
import {
	aesDecryptCTR,
	aesEncryptGCM,
	buildAckStanza,
...
import {
	...
	TC_TOKEN_INDEX_KEY
} from '../utils/auth-state-core.js'
import { cleanMessage, getHistoryMsg, makeOfflineNodeProcessor } from '../utils/message-processing.js'
import { decodeMediaRetryNode } from '../utils/media.js'
import {
	areJidsSameUser,
	binaryNodeToString,
...
import { extractGroupMetadata } from './groups-newsletter.js'
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        for (const [i, msg] of msgs.entries()) {
            if (!ids[i])
                continue;
            if (messageRetryManager?.isMessageInvalidated(remoteJid, ids[i])) {
                logger.debug({ jid: remoteJid, id: ids[i] }, 'skipping retry for edited or revoked message');
                continue;
            }
